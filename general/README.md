...
- ブラウザのコンソールでエラーを確認（F12キーで開発者ツールを開く）
- CORSエラーの場合、ローカルサーバーを使用（例: `python -m http.server 8000`）
- ファイルパスが正しいか確認（`general/output/solution.html`）

### フィルタが機能しない

- 解答情報が正しく抽出されているか確認
- `input.txt`と`assignment.txt`の形式を確認
- ブラウザのコンソールで解答情報の抽出結果を確認

### モーダルで解答が表示されない

- 解答ファイルのパスが正しいか確認
- MathJaxの読み込みエラーを確認
- モーダルの閉じるボタン（×）をクリックするか、ESCキーで閉じることができます
- ブラウザのコンソールでエラーメッセージを確認

### アーカイブされた解答が表示されない

- `general/archive/index.txt`にファイル名が正しく記載されているか確認
- ファイル名の形式が `solution_問番号_YYYYMMDD_HHMMSS.html` になっているか確認
- ブラウザのコンソールでアーカイブファイルの読み込み状況を確認
